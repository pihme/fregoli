#!/usr/bin/env python3
"""Bump fregoli from conventional commits + path filters.

Tag: fregoli/vX.Y.Z
Commit type (feat/fix/BREAKING) chooses major/minor/patch.
Only commits that touch src/, package.json, package-lock.json, tsconfig.json,
Dockerfile, or Makefile count.
"""
...
COMPONENTS = [
    {
        ...
        "asset": "fregoli-src.tar.gz",
        "build": [
            "bash",
            "-c",
            "mkdir -p dist && git archive --format=tar.gz --prefix=fregoli/ -o dist/fregoli-src.tar.gz HEAD",
        ],
    },
]
...
def main() -> int:
    ...
    for c in COMPONENTS:
        ...
        if not kind:
            touched = run(
                ["git", "rev-list", "-1", "HEAD", "--"] + c["paths"], check=False
            )
